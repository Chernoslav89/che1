...
# create nginx config
if [ ! -f /etc/nginx/sites-enabled/calambur-shop.dev ]; then
    sudo ln -s /var/www/vagrant/vhost.conf /etc/nginx/sites-enabled/calambur-shop.dev
fi

# Configuring application
echo "GRANT ALL PRIVILEGES ON *.* TO 'root'@'%' IDENTIFIED BY 'root'" | mysql -uroot -proot
echo "FLUSH PRIVILEGES" | mysql -uroot -proot
echo "CREATE DATABASE IF NOT EXISTS \`calambur_shop_dev\` CHARACTER SET utf8 COLLATE utf8_unicode_ci" | mysql -uroot -proot
